...
public static partial class EndpointApplicationSourceGeneratorTest
{
    private static readonly IReadOnlyList<MetadataReference> MetadataReferences
        ...
        [
            ...
            CreateFromType<EndpointApplicationExtensionAttribute>(),
            CreateFromType<EndpointRequest>(),
            CreateFromType<IApplicationBuilder>()
        ];

    private static GeneratedSourceResult[] RunGeneratorAndGetSources(string sourceCode, int expectedCount)
    {
        ...
        Assert.Equal(expectedCount, generatorResult.GeneratedSources.Length);
    ...
    private static CSharpCompilation CreateCompilation(string sourceCode)
    {
        return CSharpCompilation.Create(
            assemblyName: "Endpoint.AspNetCore.Generator.DynamicTests",
            syntaxTrees:
            [
    ...
    {
        var assembly = Assembly.Load("GarageGroup.Infra.Endpoint.AspNetCore.Generator");
        var generatorType = assembly.GetType("GarageGroup.Infra.EndpointApplicationSourceGenerator", throwOnError: true)!;
